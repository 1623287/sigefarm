...
function validateAndSetup() {
  ...
  if(!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    document.getElementById('username_error').innerHTML = "No debe contener espacios";
    username.focus();
  }
  else if(password.value.indexOf(' ') >= 0) {
    document.getElementById('password_error').style.display = "block";
    document.getElementById('password_error').innerHTML = "No debe contener espacios";
    password.focus();
  }
  else if(password.value.length < 6) {
    document.getElementById('password_error').style.display = "block";
    document.getElementById('password_error').innerHTML = "Debe tener una longitud de 6 o más caracteres";
    password.focus();
  }
  else if(password.value != confirm_password.value) {
    document.getElementById('password_error').style.display = "none";
    document.getElementById('confirm_password_error').style.display = "block";
    document.getElementById('confirm_password_error').innerHTML = "Clave no coincide";
    confirm_password.focus();
  }
  else {
    document.getElementById('confirm_password_error').style.display = "none";
    var confirmation = prompt("Por favor, escriba 'OK' para completar la configuración");
    if(confirmation == "OK") {
      var xhttp = new XMLHttpRequest();
    	xhttp.onreadystatechange = function() {
    		if(xhttp.readyState = 4 && xhttp.status == 200)
...
function isSetupDone() {
  ...
  if(xhttp.responseText == "true")
    window.location.href = "http://localhost/farmacia/login.php";
}
...
function verifyEmailNumber() {
  ...
  if(xhttp.responseText == "true") {
    ...
    alert("¡Correo o número de celular incorrecto!");
}
...
function updateUsernamePassword() {
  ...
  if(!notNull(username.value, 'username_error'))
    ...
    document.getElementById('password_error').style.display = "none";
    document.getElementById('confirm_password_error').style.display = "block";
    document.getElementById('confirm_password_error').innerHTML = "Contraseña no coincide";
    confirm_password.focus();
  }
  else {
    document.getElementById('confirm_password_error').style.display = "none";
    var xhttp = new XMLHttpRequest();
  	...
  	xhttp.send();
    if(xhttp.responseText == "true") {
      alert("Se configuraron las nuevas credenciales con éxito");
      displayLoginForm();
    }
    else
