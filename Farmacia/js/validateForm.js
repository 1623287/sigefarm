function notNull(text, error) {
  ...
  if(text < 0) {
    ...
    result.innerHTML = "¡Debe completar el campo!";
    return false;
  }
...
function validateName(name, error) {
  ...
  result.innerHTML = "¡Solo debe contener letras!";
...
function validateContactNumber(contact_number, error) {
  ...
  if(contact_number.length != 9) {
    result.innerHTML = "¡Debe contener 9 dígitos!";
    return false;
  }
  else
    result.style.display = "none";
...
function validateAddress(address, error) {
  ...
  if(address.trim().length < 10) {
    result.innerHTML = "¡Por favor, ingrese una dirección específica!";
    return false;
  }
  else
    result.style.display = "none";
...
function checkExpiry(date, error) {
  ...
  if(date.trim() == "" || date.trim().length != 5 || date[2] != "/")
    result.innerHTML = "¡Ingrese la fecha en formato mm/aa!";
  else if(date.slice(0, 2) < 1 || date.slice(0, 2) > 12)
    result.innerHTML = "¡Mes incorrecto!";
  else if(new Date("20" + date.slice(3, 5), date.slice(0, 2)) < new Date()) {
    result.innerHTML = "¡Medicina Expirada!";
    return -1;
  }
  else {
    result.style.display = "none";
...
function checkQuantity(quantity, error) {
  ...
  if(quantity < 0 || !Number.isInteger(parseFloat(quantity)))
    result.innerHTML = "¡Cantidad incorrecta!";
  else {
    result.style.display = "none";
    return true;
  }
...
function checkDate(date, error) {
  ...
  result.style.display = "block";
  if(date == "")
    result.innerHTML = "¡No debe estar vacío!!";
  else if(new Date(date) > new Date())
    result.innerHTML = "¡No debe usar una fecha del futuro!";
  else {
    result.style.display = "none";
    return true;
  }
